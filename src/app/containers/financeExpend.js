/**
 * Created by zhengxiuming on 2016/10/14.
 */
...
class FinanceExpend extends Component {
    constructor(props) {
        super(props);
        ...
        this.handleExpendType = this.handleExpendType.bind(this);
    }
    ...
    handleAddIncome() {
        ...
        this.openModal(<div>新增支出 <span className="header_number">编号:{this.props.number.number}</span>
        </div>, this.handleAddSubmit);
    }
    ...
    handleExpendType(val) {
        if (val) {
            if (val === "monthly") {
                $(".expendType").css("display", "block")
            } else {
                $(".expendType").css("display", "none")
            }
        }
    }
    ...
    handleCheckIncome(d) {
        ...
        var data = {};
        data.expendType = d.expend_type;
        //this.openModal("");
        this.openCheckModal(<div>收入单<span className="header_number">编号:{this.props.number.number}</span></div>, data);
    }
            ...
            content: (
                <div className="prepayment-dialog">
                    <Form onSubmit={handle}>
                        ...
                        <FormControl className="dn-select dn-required" name="belong_name" label={'所属医院：'} data={op_data}
                                     type="select" placeholder="请选择" required={true}/>
                        <FormControl className="dn-select" name="expend_time" label={'支出日期：'}
                                     type="select" placeholder="请选择"/>
                        <FormControl className="dn-select" name="expend_type" data={this.props.expendType}
                                     label={'支出类型：'} type="select"
                                     placeholder="请选择" onChange={this.handleExpendType}/>
                        <FormControl className="dn-select  expendType" name="amortize_month" label={'摊销月份：'}
                                     type="select" placeholder="请选择"/>
                        <FormControl className="dn-input dn-required account_input" name="expend_usage"
                                     label={'支出用途：'}/>
            ...
            content: (
                <div className="prepayment-dialog">
                    ...
                    <ul className="income-infoList">
                        ...
                        <li>支出日期:<span>20161013</span></li>
                        <li>支出类型:<span>{data.expendType}</span></li>
                        <li className={data.expendType==="按月摊销支出"?"amortize_month":"expendType"}>撤销月份:<span>201610-201611</span></li>
                        <li>支出用途:<span>你猜</span></li>
                        <li>操作人员:<span>****</span></li>
                        <li><span className="title-spacing">备注</span><span
                            className="colon">:</span><span>联宠国际动物检测中心</span></li>
                    </ul>
                    <div className="line-info">收入
                        <div></div>
                    </div>
                    <table className="income-bankList">
                        ...
                        {this.props.moneyAccount.list.map(
                            function (account, index) {
                                ...
                                let ele_name = "acount_name_" + account.aid;
                                return <tr>
                                    <td className="bank-name">{label_name}</td>
                                    <td>¥12,121,0</td>
                                </tr>
                            }
                        )
                    ...
                    <div className="count_info">合计收入金额:<span className="propayment-total">¥0.00</span>,<label className="expend_money">本店支出金额:<span>¥1233</span></label></div>
                </div>
            )
        })
    }
            ...
            <div>
                ...
                <div>
                    <div className="content-nav-bar clearfix">
                        <a className="new_item" onClick={this.handleAddIncome}>新增支出</a>
                        <span>财务支出汇总</span>
                        <span className="income-total">支出额总计:¥1,1000,00</span>
                    </div>
                ...
                <div className="table-content">
                    ...
                    <div className="talbe-container">
                        <Table ref="table"
                               ...
                               headers={[
                                   {name: 'create_time', header: '支出时间'},
                                   {name: 'aid', header: '支出单号'},
                                   {name: 'belong_hospital', header: '所属医院'},
                                   {name: "expend_type", header: "支出类型"},
                                   {name: "expend_usage", header: "支出用途"},
                                   {name: "market_month", header: "推销月数"},
                                   {name: 'expend_amount', header: '支出金额'},
                                   {
                                       name: 'check', header: '查看详情',
                                       content: (d)=> {
...
function mapStateToProps(state, ownProps) {
    var list = [{
        aid: "c23048018301921",
        belong_hospital: "联宠国际动物检测中心",
        expend_type: "一次性支出",
        expend_usage: "工资支出",
        market_month: "2",
        expend_amount: "¥5000.00",
...
export default connect(mapStateToProps, mapDispatchToProps)(FinanceExpend);
